...
namespace GoogleTextToSpeechApp.Pages.Home
{
    public class IndexModel : PageModel
    {
        ...
        public IActionResult OnPostConvertText()
        {
            ...
            {
                return BadRequest("Google credentials are not properly set.");
            }

            // Tạo GoogleCredential từ chuỗi JSON
            GoogleCredential googleCredential = GoogleCredential.FromJson(credentialsJson);

            // Khởi tạo Google Text-to-Speech Client với credentials đã cấu hình
            var client = TextToSpeechClient.Create(); // GoogleCredential được tự động sử dụng
